...
{% macro is_us_location(expr) -%}
    coalesce(
        ...
        or {{ expr }} ~ '\y(US|USA)\y'
        -- Strip foreign addresses whose country code is also a state code first:
        -- "Toronto, ON, CA" (not California), "Berlin, DE" (not Delaware), "Buenos Aires, AR" (not Arkansas)
        or regexp_replace(
            {{ expr }},
            ',\s*(ON|BC|AB|QC|NS|MB|SK|NB|NL|PE),\s*CA\y|\y(berlin|hamburg|munich|frankfurt|cologne|buenos aires),\s*(DE|AR)\y',
            '', 'gi'
        )
        ...
        or {{ expr }} ~* '\y(nyc|san francisco|bay area|seattle|boston|chicago|austin|los angeles|denver|atlanta|palo alto|mountain view|san jose|san mateo|menlo park|redwood city|sunnyvale|oakland|san diego|miami|dallas|houston|philadelphia|pittsburgh|minneapolis|salt lake city|phoenix|nashville|raleigh|orlando|wichita)\y',
        false
    )
{%- endmacro %}
